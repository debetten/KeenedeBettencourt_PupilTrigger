...
if 'psychopy' in sys.modules:
    from psychopy import visual, core, event, tools, parallel, monitors
...
class ExperimentalDisplay(object):
    ...
    def __init__(self,expt_design,gui_specs):
        ...
        #stimtrak
        self.stimtrak = visual.Circle(self.win, units = "cm", size = 1.25, lineColor= 0,fillColor=1, interpolate=True,edges=256,pos = (25.5,14))
        ...
        self.text_record_screen = visual.TextStim(self.win, text='Are you recording your data?',wrapWidth=10,height=.5,pos=(-16,10),color='red',fontFiles=[self.project_dir + 'display/fonts/Lato-Reg.ttf'],font=['Lato'])
        self.text_record_advance = visual.TextStim(self.win, text='Please wait for your experimenter to start the task',alignHoriz='center',wrapWidth=22,height=.75,pos=(0,1),color=(-1,-1,-1),fontFiles=[self.project_dir + 'display/fonts/Lato-Reg.ttf'],font=['Lato'])
        ...
        self.text_blink = visual.TextStim(self.win, text='Blink Aborted Trial',alignHoriz='center',wrapWidth=10,height=1,pos=(0,-1),color='red',fontFiles=[self.project_dir + 'display/fonts/Lato-Reg.ttf'],font=['Lato'])
...
class ExperimentalDesign(object):
    ...
    def __init__(self,gui_specs):
        ...
        self.env = gui_specs['env']
        self.eyetrack = gui_specs["eyetrack"]
        self.expt_date = gui_specs['expt_date']
        self.debug = gui_specs['debug']
        self.seed = gui_specs['seed'] 
        self.project_dir = gui_specs['project_dir']
        self.save_dir = gui_specs['save_dir']
        if len(self.subj_name)>5:
            self.eyetracking_filename = 'SAM' + self.subj_name[0:4] + self.subj_name[6]
        else:
            self.eyetracking_filename = 'SAM' + self.subj_name
        #print "eye tracking filename: ", self.eyetracking_filename
        np.random.seed(seed = self.seed) #seed the random number generator

        self.key_escape = ['escape']
        self.key_advance = ['space']
        self.record_key = ['grave']
        self.key_same = ['slash']
        self.key_diff = ['z']
        self.key_one = ['1']
        self.key_two = ['2']
        ...
        self.infreq_trials = (self.freq_trials==0)*1
        self.n_infreq_perblock = int(np.sum(self.freq_trials[0]==0))
        self.probetrials_dist = 2
        self.nprobetrials = 60 #trials per block 
        self.probe_trials = np.zeros((self.nblocks,self.ntrials_perblock),dtype=int)

        #eye tracking
        self.within_tol = np.ones((self.nblocks,self.ntrials_perblock)) #THE EYE TRACKING STUFF IS HERE THIS IS JUST A FLAG SO I CAN FIND IT
        self.within_tol[:] = np.nan
        ...
        self.pupil_trailingavg = np.zeros((self.nblocks,self.ntrials_perblock))
        self.pupil_runningavg = np.zeros((self.nblocks,self.ntrials_perblock))
        self.pupil_runningstd = np.zeros((self.nblocks,self.ntrials_perblock))
        self.pupil_runninglowthresh = np.zeros((self.nblocks,self.ntrials_perblock))
        self.pupil_runninghighthresh = np.zeros((self.nblocks,self.ntrials_perblock))
        self.pupil_triggered_low = np.zeros((self.nblocks,self.ntrials_perblock))
        self.pupil_triggered_high = np.zeros((self.nblocks,self.ntrials_perblock))
        self.pupil_triggered = np.zeros((self.nblocks,self.ntrials_perblock))
        self.n_pupil_triggered_low = int(self.nprobetrials/2)
        self.n_pupil_triggered_high = int(self.nprobetrials/2)
        self.eyeL = np.zeros((self.nblocks,self.ntrials_perblock))
        self.eyeR = np.zeros((self.nblocks,self.ntrials_perblock))
        self.eyeL[:] = np.nan
        self.eyeR[:] = np.nan
        self.aborted_probe = np.zeros((self.nblocks,self.ntrials_perblock))

        #set the first 80 to nan
        self.pupil_trailingavg[:,0:self.n_trials_atstart] = np.nan
        self.pupil_runningavg[:,0:self.n_trials_atstart] = np.nan
        self.pupil_runningstd[:,0:self.n_trials_atstart] = np.nan
        self.pupil_runninglowthresh[:,0:self.n_trials_atstart] = np.nan
        self.pupil_runninghighthresh[:,0:self.n_trials_atstart] = np.nan
        self.pupil_triggered_low[:,0:self.n_trials_atstart] = np.nan
        self.pupil_triggered_high[:,0:self.n_trials_atstart] = np.nan
        self.pupil_triggered[:,0:self.n_trials_atstart] = np.nan
