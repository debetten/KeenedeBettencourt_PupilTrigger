...
def get_specs(subj_name='mmddyyn_wmLoadMem02'):
    ...
    dictDlg = gui.DlgFromDict({'Participant number': '07dd19n_eyeSustAttnWM03',     #subject name: month, day, year, number, and project name
    ...
    if gui_specs['env']=='imac':
        gui_specs['project_dir']='/Users/megan/Documents/projects/eyeSustAttnWM03/'
    elif gui_specs['env']=='booth':
        gui_specs['project_dir']='C:/Users/AwhVogelLab/Desktop/Megan/eyeSustAttnWM03/'
    elif gui_specs['env']=='edslab':
        gui_specs['project_dir']='/Users/edslab/Documents/projects/eyeSustAttnWM03/'
    elif gui_specs['env']=='Dirk VU':
        gui_specs['project_dir']='C:/Users/Dirk VU/Documents/Github/eyeSustAttnWM03/'
    elif gui_specs['env']=='eeg':
        gui_specs['project_dir']='C:/Users/awhvogellab/Desktop/Megan/eyeSustAttnWM03/' #FIX THIS LATER
    else:
        pass
...
def write_to_csv_data_file(data_file,iblock,itrial,dsgn,headerLine=False):
    if headerLine:
        data_file.write('block,trial,time,setsize,freq_trials,'+
                        'colorind,correctresp,actualresp,acc,rts,right_pupil,left_pupil,'+
                        'probe_trials,pupil_triggered_high,pupil_triggered_low,'+
                        'pupil_trailingavg,pupil_runningavg,pupil_runningstd,pupil_runninglowthresh,pupil_runninghighthresh,'+
        ...
        data_file.write(str(iblock) +','+ str(itrial) +','+ datetime.utcnow().strftime("%H:%M:%S:%f") +','+ 
                        ...
                        str(dsgn.rts[iblock,itrial]) +','+ str(dsgn.eyeR[iblock,itrial]) +','+ str(dsgn.eyeL[iblock,itrial]) + ',' +
                        str(dsgn.probe_trials[iblock,itrial]) +','+ 
                        str(dsgn.pupil_triggered_high[iblock,itrial]) +','+ str(dsgn.pupil_triggered_low[iblock,itrial]) +','+ 
                        str(dsgn.pupil_trailingavg[iblock,itrial]) +','+ str(dsgn.pupil_runningavg[iblock,itrial]) +','+
                        str(dsgn.pupil_runningstd[iblock,itrial]) +','+ str(dsgn.pupil_runninglowthresh[iblock,itrial]) +','+ 
                        str(dsgn.pupil_runninghighthresh[iblock,itrial]) +','+
...
def eyetracker_settings(disp,dsgn):
    ...
    #tk = pylink.EyeLink(None)  #DELETE THIS #dummy tracker
