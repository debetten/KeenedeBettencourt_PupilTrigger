function eyeData = eyePreprocessing_encArray(subjName)
%function eyeData = eyePreprocessing(subjName)
%
%
% OUTPUTS
% - eyeData: 
...
%directory
settings.dir.eye_data_path = './../expt2/eye/raw/';
...
settings.seg.timeLockMessage = 'encArray'; % message for time locking
settings.seg.preTime = 100;  % pre-stimulus end of segment, absolute value (ms)
settings.seg.postTime = 2500; % post-stimulus end of segment, absolute value (ms)
...
if ~exist([settings.dir.eye_data_path,eye_fn],'file')
    fname = ['SAM' subjName(1:4) subjName(7)];
...
for t = 1:numel(tempTimeLockInd)
    ...
    while isempty(eyeData.trial.trialMessages{t})
        eyeData.trial.trialMessages{t} = eyeData.messages{tempTimeLockInd(t)-i}; %for the time locking message, the previous one was the trial count message
        i = i+1;
    end
end
...
%    end
    ...
    if size(eyeData.pa,1)==2
        eyeData.trial.gx(t,1,existInd) = eyeData.gx(recordedEye,tWindowInd);
        eyeData.trial.gy(t,1,existInd) = eyeData.gy(recordedEye,tWindowInd);
        eyeData.trial.hx(t,1,existInd) = eyeData.hx(recordedEye,tWindowInd);
        eyeData.trial.hy(t,1,existInd) = eyeData.hy(recordedEye,tWindowInd);
        eyeData.trial.pa(t,1,existInd) = eyeData.pa(recordedEye,tWindowInd);
        eyeData.trial.dist(t,1,existInd) = eyeData.dist(tWindowInd);
    else 
