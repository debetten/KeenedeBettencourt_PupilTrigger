...
#                parallel.setData(100+itrial%100) #trial portcode
            ...
            if response is None:
                ...
                if response is not None:
                    ...
                    if itrial>=self.dsgn.n_trials_atstart and itrial<self.dsgn.ntrials_perblock-2:
                        ...
                        self.dsgn.rts_runningslowthresh[iblock,itrial] = self.dsgn.rts_runningavg[iblock,itrial]+self.dsgn.rts_runningstd[iblock,itrial] #slow RT threshold #p of norm dist = .159
                        ...
                        # self.dsgn.rts_runningmedslowthresh[iblock,itrial] = self.dsgn.rts_runningavg[iblock,itrial]+self.dsgn.rts_runningstd[iblock,itrial]/5 #upper med RT threshold #p of norm dist = .1585
                        # self.dsgn.rts_runningmedfastthresh[iblock,itrial] = self.dsgn.rts_runningavg[iblock,itrial]-self.dsgn.rts_runningstd[iblock,itrial]/5 #lower med RT threshold
                        self.dsgn.rts_trailingavg[iblock,itrial] = np.nanmean(self.dsgn.rts[iblock,(itrial-2):(itrial+1)])
                        
                        if ~np.any(np.isnan(self.dsgn.rts[iblock,(itrial-2):(itrial+1)])) and np.all(self.dsgn.probe_trials[iblock,(itrial-2):(itrial+1)] == 0):
                            ...
                            # if self.dsgn.rts_trailingavg[iblock,itrial]<self.dsgn.rts_runningmedslowthresh[iblock,itrial] and self.dsgn.rts_trailingavg[iblock,itrial]>self.dsgn.rts_runningmedfastthresh[iblock,itrial] and np.nansum(self.dsgn.rt_triggered_med[iblock,0:itrial])<self.dsgn.n_rt_triggered_med: 
                            #     self.dsgn.rt_triggered_med[iblock,itrial] = 1
                    ...
                    if np.any(self.dsgn.within_tol[iblock,(itrial-1):(itrial+1)] == 0):
                        if self.dsgn.rt_triggered_slow[iblock,itrial] == 1 or self.dsgn.rt_triggered_fast[iblock,itrial] == 1:
                            self.dsgn.aborted_probe[iblock,itrial] = 1 
                        if self.dsgn.probe_trials[iblock,itrial]==1 or self.dsgn.rt_triggered_slow[iblock,itrial] == 1 or self.dsgn.rt_triggered_fast[iblock,itrial] == 1:
                            self.dsgn.probe_trials[iblock,(itrial):(itrial+2)] = 0
                            self.dsgn.rt_triggered[iblock,itrial] = 0
                            self.dsgn.rt_triggered_fast[iblock,(itrial-1):(itrial+1)] = 0
                            self.dsgn.rt_triggered_slow[iblock,(itrial-1):(itrial+1)] = 0
        ...
        if response is None:
            if itrial>=self.dsgn.n_trials_atstart:
                ...
                self.dsgn.probe_trials[iblock,itrial] = 0
                self.dsgn.rt_triggered[iblock,itrial] = 0
                self.dsgn.rt_triggered_slow[iblock,itrial-1] = 0
                self.dsgn.rt_triggered_fast[iblock,itrial-1] = 0
    ...
    def retentionInterval(self,iblock,itrial,clock,tk=None):
        ...
        if self.dsgn.within_tol[iblock,itrial]==0:
            ...
            self.dsgn.rt_triggered[iblock,itrial] = 0
            self.dsgn.rt_triggered_fast[iblock,(itrial-2):(itrial+1)] = 0
            self.dsgn.rt_triggered_slow[iblock,(itrial-2):(itrial+1)] = 0
            self.disp.fix.fillColor = (-1,-1,-1)
            self.disp.fix.draw()
            self.disp.text_blink.draw()
            self.disp.win.flip()
    ...
    def memoryProbe(self,iblock,itrial,clock,f=None,tk=None):
        ...
        if self.disp.env=='eeg':
            if itrial<80:
                ...
                print 34
            elif self.dsgn.rt_triggered_fast[iblock,itrial-1]==1: #rt triggered fast portcode
                parallel.setData(31)
                print 31
            elif self.dsgn.rt_triggered_slow[iblock,itrial-1]==1: #rt triggered slow portcode
                parallel.setData(32)
                print 32
        ...
        if tk is not None:
            if self.dsgn.rt_triggered_fast[iblock,itrial-1]==1:
                tk.sendMessage('fastTrigger')
            if self.dsgn.rt_triggered_slow[iblock,itrial-1]==1:
                tk.sendMessage('slowTrigger')
